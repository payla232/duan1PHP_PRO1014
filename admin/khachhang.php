...
if(isset($_GET['xoa'])) {
    pdo_query("DELETE FROM `orders` WHERE `id` = '".$_GET['xoa']."' ");
    echo '<script>location.href="/duan1/admin/khachhang.php"</script>';
}
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <aside style="background-color: #4793af; padding-top: 20px; width: 100%">
                    <div class="table" style="width: 100%">
                        <table>
                            <thead>
                                <tr>
                                    <th class="th" colspan="7">Lịch sử đơn hàng</th>
                                </tr>
                                <tr>
                                    <th>STT</th>
                                    <th>Mã đơn hàng</th>
                            ...
                            <tbody>
                                <?php $i = 1; foreach (pdo_query("SELECT * FROM `orders` ORDER BY `id` DESC ") as $user):
                                    extract($user);    
                                ?>
                                <tr>
                                    <td><?= $i++ ?></td>
                                    <td><?= $trading ?></td>
                                    <td><?= $username ?></td>
                                    <td><?=pdo_query_one("SELECT * FROM `pro_pub` WHERE `id` = '".$id_pro."' ")['name'];?></td>
                                    <td><?= $amount ?></td>
                                    <td><?= statusOrders($status) ?></td>
                                    <td class="tdd">
                                        <button class="btn_sua">
                                            <a href="suaOrder.php?id=<?= $id?>">
